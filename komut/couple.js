...
member.roles.add(ability.couple)
message.react(config.onay)
client.channels.cache.get(kanallar.MessageLogs).send(`Bir komut kullanıldı! komut\n kullanan yetkili: \`${message.author.tag}\` - (\`${message.author.id}\`)\n Kullanılan komut: \`.sevgilimvar\``);
}

module.exports.conf = { enable: true, guildOnly: true, aliases: ['couple', 'lovely', 'love'] }

module.exports.help = { name: 'couple' }