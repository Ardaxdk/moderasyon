...
let x = message.guild.members.cache.filter(x => db.has(`${message.guild.id}.${x.id}.tagSorgu_`))
...
let obj = []
let kaan = 0
let sorted = x.sort((a,b) => db.get(`${message.guild.id}.${b.id}.tagSorgu_`) - db.get(`${message.guild.id}.${a.id}.tagSorgu_`)).map((r,index) => `**  ${++kaan}. ${r.user}** - (\`${r.user.id}\`) **${db.get(`${message.guild.id}.${r.id}.tagSorgu_`)}** Taglı Kazandırmış `).slice(0, 10)
...
exports.conf = { enabled: true, aliases: ['top-tag-liste'], permLevel: 0 };

exports.help = { name: 'tta', usage: `toptaglı` }